  ...
  return (
    <section className="px-6 py-24 lg:px-10 lg:py-32">
      <Reveal className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-5 font-display text-2xl font-medium italic text-ink lg:text-3xl">
      ...
      <div className="mx-auto mt-12 grid max-w-7xl grid-cols-1 gap-6 lg:grid-cols-3">
        {EXPLORE_MODES.map((mode, i) => (
          <Reveal key={mode.id} delay={i * 120} className="h-full">
            <Link
              href={mode.href}
              className="group flex h-full flex-col gap-6 border border-ink/10 bg-surface/40 p-8 transition-colors hover:bg-surface"
              style={{ borderTopWidth: "3px", borderTopColor: mode.accent }}
            >
              <div>
                <p className="type-pre-title text-ink/60">{mode.category}</p>
                <p className="mt-1 font-display text-4xl font-bold text-ink/25">
                  {mode.number}
                </p>
              </div>
              <h3 className="font-display text-3xl font-medium leading-[1.05] tracking-[-0.01em] text-ink">
                {mode.title}
              </h3>
              <p className="type-body text-ink/65">{mode.description}</p>
